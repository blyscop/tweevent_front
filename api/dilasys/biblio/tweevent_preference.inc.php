...
*/
function Tweevent_preference_recuperer($id_tweevent_preference, $nom_preference)
{
	$tweevent_preference = new Tweevent_preference();

	if($id_tweevent_preference > 0 && empty($nom_preference)) {
		...
		$sql = " SELECT *
				FROM " . $GLOBALS['prefix'] . "tweevent_preference
				WHERE libelle_tweevent_preference = '$nom_preference';";
	}
...
function Tweevent_preferences_chercher($args)
{
	...
	if (isset($args['tab_ids_tweevent_preferences_categories']) && $args['tab_ids_tweevent_preferences_categories'] != "*") {
		$ids = implode(",", $args['tab_ids_tweevent_preferences_categories']);
		$condition .= " AND id_tweevent_categorie IN (0".$ids.") ";
	}
	if (!isset($args['etat']))
