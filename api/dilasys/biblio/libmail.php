<?php
if (!defined('__LIBMAIL_INC__')){
define('__LIBMAIL_INC__', 1);
/*
...
function Attach( $filename, $filenamelocal, $filetype = "", $disposition = "inline" )
{
...
//	$this->xheaders['To'] = implode( ", ", $this->sendto );
	...
	if( $this->charset != "" ) {
		...
		$this->xheaders["Content-Type"] = "text/html; charset=$this->charset"; // JPLt
...
function _build_attachement()
{
	...
	$tab_names = array(); // JPLt
	for( $i=0; $i < count( $this->aattach); $i++ ) {
		$filename = $this->aattach[$i];
		$tab_names[] = basename($filename);
	}
	if (count($tab_names)) {
		$attach_names = implode(',', $tab_names);
		$this->fullBody .= "X-attachments: ".$attach_names."\n";
	}
...
} // Fin if (!defined('__LIBMAIL_INC__')){
?>
